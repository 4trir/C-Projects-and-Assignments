...
public:
    ...
    void sort() override;
    int search(const T target) override;
...
void Array<T>::sort() {
    ...
    for (int i = 0; i < _size / 2; i++) {
        leftHalf.insert(i, _pArray[i]);
    }
    for (int i = _size / 2; i < _size; i++) {
        rightHalf.insert(i - _size / 2, _pArray[i]);
    }
    leftHalf.sort();
    rightHalf.sort();
    ...
    while (i < leftHalf.size() && j < rightHalf.size()) {
        if (leftHalf.get(i) <= rightHalf.get(j)) {
            _pArray[k] = leftHalf.get(i);
            i++;
        } else {
            _pArray[k] = rightHalf.get(j);
            j++;
        }
        k++;
    }
...
template <typename T>
int Array<T>::search(const T target) {
    int low = 0;
    int high = _size - 1;
    ...
    while (low <= high) {
        ...
        if (_pArray[mid] == target) {
            return mid;
        } else if (_pArray[mid] < target) {
            low = mid + 1; 
        } else {
            high = mid - 1; 
        }
